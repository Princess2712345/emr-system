...
const confinementRecords = ref([
  ...
  { id: 2, patientName: 'Alice Smith', diagnosis: 'Pneumonia', roomNumber: '205-B', admitDate: 'Oct 22, 2023', dischargeDate: null, duration: '2 Days (Current)', status: 'Admitted' },
  { id: 3, patientName: 'Robert Johnson', diagnosis: 'Post-Op Monitoring', roomNumber: 'ICU-04', admitDate: 'Oct 23, 2023', dischargeDate: null, duration: '1 Day (Current)', status: 'Observation' },
  { id: 4, patientName: 'Sarah Williams', diagnosis: 'Dengue Fever', roomNumber: '310-C', admitDate: 'Oct 15, 2023', dischargeDate: 'Oct 20, 2023', duration: '5 Days', status: 'Discharged' }
])
...
const viewSummary = (record) => alert(`Loading Summary: ${record.patientName}`)
...
/* --- FIXED LOGO STYLES (Match icons & text perfectly) --- */
.sidebar-logo { 
  display: flex; 
  align-items: center; 
  gap: 12px; 
...
.logo-icon { 
  color: #60a5fa; 
  font-size: 24px !important; /* Fixed size */
  width: 24px;
  height: 24px;
  flex-shrink: 0;
}
.logo-text { 
  font-size: 20px; /* Scaled to visually match the icon thickness */
  font-weight: 800; 
  line-height: 1; 
  color: white;
...
.search-wrapper input { width: 100%; padding: 0.85rem 1rem 0.85rem 3rem; border: 1px solid #e2e8f0; border-radius: 12px; outline: none; background: white; box-sizing: border-box; }
...
.form-group input, .modal-select, textarea { width: 100%; padding: 0.8rem; border: 1px solid #e2e8f0; border-radius: 10px; outline: none; box-sizing: border-box; }
